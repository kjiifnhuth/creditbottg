...
async function importBackupFileToCloud(authUser, file){
  ...
  // Якщо у backup один користувач, імпортуємо його незалежно від email.
  // Це потрібно, коли старий локальний акаунт мав іншу адресу, ніж новий Supabase-акаунт.
  const normalizedUsers=backup.users.filter(Boolean);
  const matchedUser=normalizedUsers.find(u=>(u?.email||'').trim().toLowerCase()===targetEmail);
  const sourceUser=matchedUser || (normalizedUsers.length===1 ? normalizedUsers[0] : null);
  if(!sourceUser){
    const available=normalizedUsers.map(u=>u?.email).filter(Boolean).join(', ');
    throw new Error(`Не вдалося визначити дані для імпорту. У backup знайдено користувачів: ${available || 'невідомо'}.`);
  }
...
function App(){
  ...
  return <div className="app"><aside className={`sidebar ${menu?'open':''}`}><div className="side-brand"><div className="logo small"><CreditCard/></div>Кредитний менеджер</div><nav><Nav icon={<Home/>} text="Огляд" active={view==='dashboard'} click={()=>{setView('dashboard');setMenu(false)}}/><Nav icon={<CreditCard/>} text="Мої кредити" active={view==='credits'} click={()=>{setView('credits');setMenu(false)}}/><Nav icon={<WalletCards/>} text="Кредитні ліміти" active={view==='limits'} click={()=>{setView('limits');setMenu(false)}}/><Nav icon={<CalendarDays/>} text="Платежі" active={view==='payments'} click={()=>{setView('payments');setMenu(false)}}/></nav><div className="side-bottom"><Nav icon={<Settings/>} text="Налаштування" active={view==='settings'} click={()=>{setView('settings');setMenu(false)}}/><Nav icon={<LogOut/>} text="Вийти" click={logout}/></div></aside>{menu&&<div className="backdrop" onClick={()=>setMenu(false)}/>}<div className="main"><header className="header"><button className="mobile-menu" onClick={()=>setMenu(true)}><Menu/></button><div className="search"><Search/><input placeholder="Пошук..."/></div><div className="header-actions"><button className="round" onClick={toggleTheme}>{state.theme==='dark'?<Sun/>:<Moon/>}</button><div className="avatar">{user.name?.[0]?.toUpperCase()||'К'}</div></div></header><main className="page">
{syncError&&<div className="sync-alert">{syncError}</div>}
{localCandidate&&<div className="sync-alert"><b>Знайдено локальні дані з цього браузера.</b> <button onClick={migrateLocal}>Імпортувати в акаунт</button></div>}
